...
line1=()
[ -n "$vim_mode" ] && line1+=("${vim_mode}")
[ -n "$account" ] && line1+=("${account}")
line1+=("${CYAN}${short_cwd}${RESET}")
[ -n "$repo_link" ] && line1+=("${repo_link}")
if [ -n "$transcript_path" ]; then
  session_link="${BRIGHT_BLUE}\e]8;;file://${transcript_path}\a${session_short}\e]8;;\a${RESET}"
else
  session_link="${BRIGHT_BLUE}${session_short}${RESET}"
fi
line1+=("${session_link}" "${duration} | ${cost}")
