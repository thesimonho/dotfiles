...
export function App() {
  ...
  useEffect(() => {
    fetch("/api/workloads")
      ...
      .then(setWorkloads)
      .catch(() => setErrorMessage("Workload status is temporarily unavailable."));
  }, []);
  ...
  return <>
    ...
    <section aria-labelledby="workloads-heading" className="panel">
      ...
      <ul className="workload-grid">
        {workloads.map((workload) => <li className="workload-card" key={`${workload.namespace}/${workload.name}`}>
