...
Prepared profiles are hook-inclusive so evaluated behaviour matches day-to-day sessions instead of measuring prose that never runs alone. The Claude profile receives a `settings.json` containing only the live `hooks` block; other live settings stay out of the arm. The Codex profile receives the live `hooks.json`, and the copied `config.toml` drops `[hooks.state]` trust entries because they are keyed by the live path and cannot match the temporary profile; `codex exec` instead passes `--dangerously-bypass-hook-trust`, which is safe here because the harness itself copies the vetted hook sources. Both preparations fail loudly rather than produce a silently hook-free profile; a profile is hook-free only when its variant deliberately carries no hook components, which is exactly the naked arm. Verified empirically: a blocking hook (`block-force-push`) denies its command from inside a prepared profile for Claude, and Codex acknowledges enabled hooks under the bypass flag. Runs before 2026-08-07 predate hook inclusion and measured instruction-only adherence.
