...
The catalog uses eight HomeOps scenarios: a small debugging repair, authorized GitOps remediation, large maintenance feature, read-only diagnosis, template exploration, isolated worktree handoff, secret-backed integration, and critical authentication discovery. Cases emit only the assessments whose evidence and applicability they declare. One MLflow scorer returns a list of named `Feedback` objects, so MLflow aggregates the same assessment name across applicable rows without inventing failures for inapplicable behavior.

The public suite recipes express cost intent rather than a separate case catalog: `eval-smoke` is the two-case harness-development check, `eval-core` is the normal seven-case evaluation suite, and `eval-extended` adds the expensive worktree lifecycle case. `eval-run` remains the normal-suite alias for `eval-core`. Every tier remains a filtered view of the hosted `agent-harness-cases` dataset, so MLflow does not create a generic `dataset` identity for subset runs.
