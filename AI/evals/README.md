...
When a run omits top-level compute settings, the harness defaults both the evaluated base agent and any judges to `gpt-5.6-terra` with `medium` effort for Codex or `sonnet` with `medium` effort for Claude. Explicit model and effort arguments in the `just` recipes, or direct `--model` and `--effort` flags, override either default independently so runs can compare settings such as low versus medium effort. The selected top-level settings become the baseline for the **Subagent compute selection** case; child selections are judged as relative delegation or escalation rather than against fixed model pairs.
