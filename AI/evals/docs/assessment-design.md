...
- Frank usage: large cases require an agent-spawn event plus the configured Frank definition canary in the loaded agent context. Small cases require zero Frank delegations, judged on that same identity evidence rather than on spawn events, because delegating simple work to a cheaper or lower-effort child is the compute judgement the guidance asks for and is not planning overhead. A matching nickname does not count as the configured agent. Each CLI proves identity differently. Claude proves it with the canary footer, read below the continuation hint and usage block the CLI appends under a subagent's reply. Codex proves it with the role recorded in the child's own session rollout, because the parent transcript does not always carry the spawn item; a resolved child is itself proof of delegation, so it also supplies the spawn event.
