...
| `dataset_sync.py` | Converts cases to MLflow records with `case_name` as an input (not an expectation), then replaces dataset contents without exposing metric definitions as dataset expectations |
...
| `mlflow_records()` / `sync_mlflow_dataset()` | `dataset_sync.py` | Serialize local case inputs, including `case_name`, and make the remote dataset match the complete local catalog |
