...
| `compute_selection_scoring.py` | Maintains least-to-most-capable model/effort ladders per CLI and scores child selections as delegation or escalation relative to the evaluated parent compute |
...
| `score_compute_selection()` | `compute_selection_scoring.py` | Awards half credit for a child below the current parent baseline and half for a child above it; explicit child effort selects one ladder position, while Claude model-only evidence compares the model's entire effort band |
